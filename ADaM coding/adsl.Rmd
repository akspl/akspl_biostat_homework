...
sl_ds <- 
  ...
  select(STUDYID, SUBJID, SITEID, 
         ...
         SEX, RACE, 
         ARM) %>% 
  mutate(USUBJID = str_c(STUDYID, SUBJID, sep = "-"),
         AGE = as.numeric(AGE),
         TRTP = ARM, 
         #grepl если есть паттерн '' в векторе то присваиваем ~ что-то
         TRTPN = case_when(grepl("0", TRTP) ~ 1,
                           grepl("1", TRTP) ~ 2))
...
trt_dates <- ex_ds %>% 
  ...
  mutate(L = last(EXSEQ[EXOCCUR == 'Yes' & 
                          !is.na(EXOCCUR)])) %>% 
  ungroup() %>% 
...
trt_dates <- trt_dates %>% 
  #определяем длительность приема препарата
...
vs_res <- 
  vs_ds %>% 
  #выберем измерение роста веса и .. на визите 0
...
su_ds <- 
  ...
  mutate(SUDOSTXT = case_when(SUDOSTXT == "Current user" ~ "Yes",
                              SUDOSTXT == "Ex-user" ~ "Ex-user",
                              SUDOSTXT == "Non-user" ~ "No")) %>%
  pivot_wider(id_cols = SUBJID, 
              names_from = SUCAT, 
              values_from = SUDOSTXT) %>% 
  rename(SMOKSTAT = `Tobacco Use`,
         ALCSTAT = `Alcohol Use`)
...
doreas <- 
  ...
  select(SUBJID, DOREAS)

dcsreas <- 
  supp_disp %>% 
  filter(DSSEQ == 3) %>% 
  mutate(DCSREAS = DSREAS) %>% 
  select(SUBJID, DCSREAS, DSSTATUS)
...
compl_fl <- supp_disp %>% 
  filter(DSSEQ == "3") %>%
  mutate(COMPLFL = ifelse(DSSTATUS == "Subject discontinued the study", 
                          "N", "Y"))
...
#приводим все переменные к написанию с заглавной буквы
pop_list <- pop_list %>%
  mutate_at(vars(PP, FAS, SAF), str_to_title)

pop_list <- pop_list %>% 
  mutate(PPROTFL = case_when(PP == "Yes" ~ "Y",
                           PP == "No" ~ "N"),
         FASFL = case_when(FAS == "Yes" ~ "Y",
                           FAS == "No" ~ "N"),
         SAFFL = case_when(SAF == "Yes" ~ "Y",
                           SAF == "No" ~ "N")
         ) %>% 
  select(SUBJID, 
         PPROTFL, PPXL,
